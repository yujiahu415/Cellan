...
class AnalyzeCells():
	...
	def __init__(self,path_to_file,results_path,path_to_detector,cell_kinds,detection_threshold=None,expansion=None,fov_div=1):
		...
		if os.path.splitext(os.path.basename(self.path_to_file))[1] in ['.tif','.TIF','.tiff','.TIFF','.svs','.SVS']:
			self.lif=False
		else: 
			self.lif=True
		...
		if self.lif:
			...
			for w in range(self.fov_div):
				...
				for h in range(self.fov_div):
					...
					detect_fov=np.uint8(exposure.rescale_intensity(image[h*fov_height:(h+1)*fov_height,w*fov_width:(w+1)*fov_width],out_range=(0,255)))
